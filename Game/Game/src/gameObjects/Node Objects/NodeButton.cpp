...
void NodeButton::initGameObject(Node* nodeIt, CallBack _cb, Vector2D pos, float navigatorhorizontalMult, std::function<void(Transform*)> onSel) {

	addComponent<Transform>(pos, VECTOR_ZERO, NODE_WIDTH, NODE_HEIGHT);
	...
	addComponent<NodeButtonComponent>(_cb, nodeIt->getState(), navigatorhorizontalMult)->setOnSelected(onSel);
