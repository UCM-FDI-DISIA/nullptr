...
void PulgaCard::attack(Vector2D playerPos, Vector2D mousePos, float attackMult, BattleScene* where)
{
	...
	Hitbox::HitboxData data = {playerPos, dir * BULLET_SPEED, Vector2D(data.width / 2, data.height / 2), 16, 16, 0, "Bullet", _grp_ENEMIES};
...
{
	Hitbox::HitboxData data = { playerPos, VECTOR_ZERO, Vector2D(data.width / 2, data.height / 2), 800, 800, 0, "FlashBang", _grp_ENEMIES };
