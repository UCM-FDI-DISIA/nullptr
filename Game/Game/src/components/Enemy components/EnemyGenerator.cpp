...
void EnemyGenerator::spawnWave(const int wave[3]) {
	...
	for (int i = 0; i < wave[1]; i++) {
		...
		GameObject* enemy = where->addGameObject<RangedEnemy>(_grp_ENEMIES,spawnPos, 10, player, where);
	}
	...
	for (int i = 0; i < wave[2]; i++) {
		...
		GameObject* enemy = where->addGameObject<TankEnemy>(_grp_ENEMIES,spawnPos, 10, player, where);
	}
}
