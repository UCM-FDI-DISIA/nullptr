...
void RangeBehaviour::enemyAttack() {
	...
	if (vel.magnitude() != 0) {
		...
		Hitbox::HitboxData data = { pos->getPos(), vel * BULLET_SPEED, Vector2D(data.width / 2, data.height / 2), 30, 30, 0, "Bullet", _grp_PLAYER };
		/*vel = vel * bulletSpedd;*/
		if (shotPattern == 0) {
			...
			data = { pos->getPos(), vel * BULLET_SPEED, Vector2D(data.width / 2, data.height / 2), 30, 30, 0, "Bullet", _grp_PLAYER };
			gStt->addGameObject<Hitbox>(_grp_ENM_ATTACK, damage, true, false, 10, data);
