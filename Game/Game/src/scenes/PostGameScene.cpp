...
PostGameScene::PostGameScene(int dmg, int mlee, int rngd, int tnk, int mny, float tm) : damage(dmg), melee(mlee), ranged(rngd), tank(tnk), money(mny), time(tm) {
	...
	title->addComponent<Transform>(Vector2D(70, 65), VECTOR_ZERO, 400, 100);
	title->addComponent<TextComponent>(&sdlutils().fonts().at("ARIAL48"), "ESTADISTICAS");

	GameObject* enemiesKilled = addGameObject(_grp_UI);
	enemiesKilled->addComponent<Transform>(Vector2D(70, 155), VECTOR_ZERO, 400, 100);
	...
	time->addComponent<Transform>(Vector2D(70, 335 + 60 * i), VECTOR_ZERO, 400, 100);
	std::stringstream ss;
	...
	money->addComponent<Transform>(Vector2D(70, 425 + 180), VECTOR_ZERO, 400, 100);
