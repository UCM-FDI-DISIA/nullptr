...
BattleScene::BattleScene(BattleType t_) : NodeScene(), type(t_) {
	...
	background3->addComponent<Image>(SDLApplication::getTexture("BattleBackground3"))->setScrollFactor(Constant::getFloat("BATTLEBACKGROUND3_SCROLLFACTOR"));
	background2 = addGameObject();
	background2->addComponent<Transform>(Vector2D(), Vector2D(), Constant::getFloat("BATTLEBACKGROUND123_WIDTH"), Constant::getFloat("BATTLEBACKGROUND123_HEIGHT"));
	background2->addComponent<Image>(SDLApplication::getTexture("BattleBackground2"))->setScrollFactor(Constant::getFloat("BATTLEBACKGROUND2_SCROLLFACTOR"));
	background1 = addGameObject();
	background1->addComponent<Transform>(Vector2D(), Vector2D(), Constant::getFloat("BATTLEBACKGROUND123_WIDTH"), Constant::getFloat("BATTLEBACKGROUND123_HEIGHT"));
	background1->addComponent<Image>(SDLApplication::getTexture("BattleBackground1"))->setScrollFactor(Constant::getFloat("BATTLEBACKGROUND1_SCROLLFACTOR"));
	
	//Suelo
	floor = addGameObject();
	floor->addComponent<Transform>(Vector2D(50, 50), Constant::getVector2D("FLOOR_PAST_VELOCITY"), Constant::getFloat("FLOOR_WIDTH"), Constant::getFloat("FLOOR_HEIGHT"));
	...
	//Música
	battleSceneOST = &sdlutils().musics().at(Constant::getString("BATTLE_MUSIC"));
	battleSceneOST->play();
};
...
void BattleScene::changeToGameOverScene() {
	...
	delay->addComponent<CallbackDelayer>([&]() {
		...
		SDLApplication::pushNewScene<GameOverScene>(); }, Constant::getInt("DEATH_DELAY"));
}
